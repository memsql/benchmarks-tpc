...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast('2001-08-25' as date)
                  and (timestampadd(day,14, cast('2001-08-25' as date)))
...
select case when (select count(*)
                  from store_sales
                  where ss_quantity between 1 and 20) > 9159161
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 4505784
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 3077724
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 8325216
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 43481517
            then (select avg(ss_ext_list_price)
                  from store_sales
...
select
 ...
 where
  ...
  ca_county in ('Tom Green County','Nassau County','Gosper County','Boone County','Tishomingo County') and
...
where
        ...
        and d_date between cast('1998-01-29' as date)
                and (timestampadd(day,30, cast('1998-01-29' as date)))
group by
        i_item_id
        ,i_item_desc
        ,i_category
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('WI', 'TN', 'NV')
  and ss_net_profit between 100 and 200
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('NM', 'NH', 'CO')
  and ss_net_profit between 150 and 300
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('TX', 'MN', 'UT')
...
from
  ...
  ,call_center
where
    d_date between '2000-5-01'
            and (timestampadd(day,60, cast('2000-5-01' as date)))
and cs1.cs_ship_date_sk = d_date_sk
and cs1.cs_ship_addr_sk = ca_address_sk
and ca_state = 'CO'
and cs1.cs_call_center_sk = cc_call_center_sk
and cc_county in ('Bronx County','Jefferson Davis Parish','Luce County','Richland County',
                  'Marshall County'
)
and exists (select *
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (8,10,4,11,9,6) and
       d_year = 2002 and
       ca_state in ('TN','IN','TX'
                   ,'IA','WV','ND','MS')
...
select  i_item_desc
 ...
 and d_date between cast('1998-06-22' as date)
                and (timestampadd(day,30, cast('1998-06-22' as date)))
 group by i_item_id
         ,i_item_desc
         ,i_category
         ,i_class
...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between
                 (timestampadd(day,-30, cast('1998-03-08' as date)))
                and (timestampadd(day,30, cast('1998-03-08' as date)))
...
and d_date between '2001-01-25' and
         (timestampadd(day,90, cast('2001-01-25' as date)))
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Walker County','Bronx County','Levy County','Oglethorpe County',
                           'Wadena County','Luce County','Pennington County','San Miguel County')
...
select  i_item_id
 ...
 and d_date between cast('1999-04-17' as date) and (timestampadd(day,60, cast('1999-04-17' as date)))
 and i_manufact_id in (952,696,717,829)
...
select
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (timestampadd(day,-30, cast('1999-05-26' as date)))
                and (timestampadd(day,30, cast('1999-05-26' as date)))
 group by
    w_state,i_item_id
 ...
 where i_manufact_id between 732 and 732+40
   and (select count(*) as item_cnt
       ...
       (i_manufact = i1.i_manufact and
        ...
        (i_category = 'Men' and
        (i_color = 'floral' or i_color = 'lawn') and
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Lakeside','Five Points','Oakland','Pleasant Grove','Salem')
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('cornflower','misty','blue'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 1999
 and     d_moy                   = 3
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('dim','forest','puff','metallic','sandy','navajo') and
 ...
 from (
    (select
     ...
     where
        ...
        and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
        and t_time between 57134 and 57134+28800
...
select  c_last_name
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and date_dim.d_year in (1998,1998+1,1998+2)
        and store.s_city in ('Riverside','Harmony')
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (2000,2000+1,2000+2)
    and store.s_county in ('Dauphin County','Marshall County','Levy County','Pennington County')
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2000-08-16' as date)
                  and (timestampadd(day,30, cast('2000-08-16' as date)))
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('1999-08-14' as date)
                  and (timestampadd(day,30, cast('1999-08-14' as date)))
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2001-05-20' as date) and (timestampadd(day,60, cast('2001-05-20' as date)))
 and i_manufact_id in (450,443,565,313)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in
        ...
        where d_week_seq in
                (select d_week_seq
                from date_dim
        where d_date in ('1999-03-15','1999-09-14','1999-11-21')))
...
select  c_customer_id as customer_id
 ...
 where ca_city  =  'Unionville'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  21646
   and ib_upper_bound   <=  21646 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (2000) and
        ((i_category in ('Children','Shoes','Jewelry') and
          i_class in ('infants','mens','gold')
         )
      or (i_category in ('Home','Women','Sports') and
          i_class in ('glassware','dresses','fishing')
        ))
...
i_manufact_id = 642
and i_item_sk = ws_item_sk
and d_date between '2002-03-04'
        and (timestampadd(day,90, cast('2002-03-04' as date)))
...
from
  ...
  ,web_site
where
    d_date between '2001-2-01'
            and (timestampadd(day,60, cast('2001-2-01' as date)))
...
from
  ...
  ,web_site
where
    d_date between '1999-3-01'
          and (timestampadd(day,60, cast('1999-3-01' as date)))
and ws1.ws_ship_date_sk = d_date_sk
and ws1.ws_ship_addr_sk = ca_address_sk
and ca_state = 'PA'
and ws1.ws_web_site_sk = web_site_sk
...
where
        ...
        and d_date between cast('1999-04-03' as date)
                and (timestampadd(day,30, cast('1999-04-03' as date)))
group by
        i_item_id
        ,i_item_desc
        ,i_category
